...
void MainMenu()
{
  ...
  Window *window;
  int done = 0;
  temp = IMG_Load("images/AlienRushSplashScreen.png");
  if(temp == NULL)
  {
  ...
  LoadMouse();
  window = NewWindow("Main Menu",NULL,10,90, 200,200,LightGrey,Red,0,DrawWindowGeneric,NULL);
  NewWindowButton(window,NewButton(NULL,"New Game",20,130, 100, 60,SDLK_n,0,Blue,White));
  do
  {
    ResetBuffer();
    SDL_PumpEvents();
	...
	else if(keys[SDLK_ESCAPE] == 1 )
	{
		printf("You ended the game by pressing escape\n");
		exit(1);
	}
    NextFrame();
  }
...
void UpdateMenu()
{
	if(Mouse.oldbuttons & SDL_BUTTON(1))
	{
	if(MouseIn(20,130,100,60))
	 {	
		 lvl = 1;
		 printf("about to change lvl number");
	 }
	}
}
...
void LoadDashboard()
{
  ...
  temp = IMG_Load("images/gamesprites/dashboard.png");
  if(temp == NULL)
  {
    fprintf(stderr,"unable to load dashboard: %s\n",SDL_GetError());
    exit(0);
  ...
  Dashboard.tiles = LoadSprite("images/tilesetlv1.png",40,40);
