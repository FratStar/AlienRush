...
void SpawnSpriteParticle(char filename[40],int w,int h,int frames,int sx,int sy,float vx,float vy,Uint8 time)
{
  ...
  if(newparticle != NULL)/*lets always make sure we have a particle*/
  {
    ...
    SDL_SetColorKey(newparticle->sprite->image, SDL_SRCCOLORKEY, SDL_MapRGBA(newparticle->sprite->image->format,0,0,0,255));
...
void ExplodingParticle(int sx,int sy,float vx,float vy)
{
  ...
  char text[40];
  sprintf(text,"images/weaponsprites/explosion%i.png",(rand()%2) + 1);
