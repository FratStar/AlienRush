...
void SpawnFlyer(Entity *owner,int x,int y,int weapon,int affiliation)
{
  ...
  strcpy(newent->EntName,"flyer\0");
  newent->sprite = LoadSprite("images/enemies/flyer.png",38,59);
 /* newent->legs = NULL;*/
  /*SDL_SetColorKey(newent->sprite->image, SDL_SRCCOLORKEY , SDL_MapRGB(newent->sprite->image->format, 0,0,0));*/
  newent->owner = owner;
  newent->update = UpdateFlyer;
  newent->think = EnemyThink;
  newent->UpdateRate = 45;
  newent->ThinkRate = 45;
  newent->Unit_Type = affiliation;
