...
/*Entity __entityList[MAXENTITIES];  /*the last column is the world*/
...
void ClearEntitiesExcept(Entity *skip)
{
  ...
  {
    if((__entityList[i].used)&&(skip != &__entityList[i]))
    {
...
void DrawBBoxEntities()
{
  ...
  for(i = 0;i < MAXENTITIES;i++)
  {
    if(__entityList[i].used == 1)
    {
      if(((__entityList[i].s.x + __entityList[i].size.x) >= Camera.x)&&(__entityList[i].s.x <= (Camera.x + Camera.w))&&((__entityList[i].s.y + __entityList[i].size.y) >= Camera.y)&&(__entityList[i].s.y <= (Camera.y + Camera.h)))
      {
        DrawRect(__entityList[i].Boundingbox.x + (int)__entityList[i].s.x - Camera.x,__entityList[i].Boundingbox.y + (int)__entityList[i].s.y - Camera.y, __entityList[i].Boundingbox.w, __entityList[i].Boundingbox.h, IndexColor(__entityList[i].Color), screen);
      }
      count++;
    }
  }
...
Entity *NewEntity()
{
  ...
  for(i = 0;i < MAXENTITIES;i++)
  {
	...
	return NULL;
  //__entityList[i].EntClass = EC_AI;
  //__entityList[i].totaloffset = 0;
...
int CollisionDetect(Entity *owner, Entity *target)
{
	...
	bbox.x = (int)owner->s.x;
	bbox.y = (int)owner->s.y;
	bbox.w = owner->Boundingbox.w;
	bbox.h = owner->Boundingbox.h;
	
	bbox2.x = target->s.x;
	bbox2.y = target->s.y;
	bbox2.w = target->Boundingbox.w;
	bbox2.h = target->Boundingbox.h;

	/*Calculate the sides of the first rect*/
	left = bbox.x;
	right = bbox.x + bbox.w;
	top = bbox.y;
	bottom = bbox.y + bbox.h;

	/*Calculate the sides of the second rect*/
	left2 = bbox2.x;
	right2 = bbox2.x + bbox2.w;
	top2 = bbox2.y;
	bottom2 = bbox2.y + bbox2.h;
